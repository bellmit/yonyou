...
public class ForecastImportController {
	...
	* @throws
    ...
    public ResponseEntity<TmpVsMonthlyForecastDTO> retailforecastIssued(
        ...
        TmpVsMonthlyForecastDTO dto = new TmpVsMonthlyForecastDTO();
        ...
        dto.setErrorMsg(year+"年 第"+month+"生产订单任务下发完成，任务编号:"+taskId);
	...
	* @throws
    ...
    @ResponseBody
	public List<OrganizationTreeDto> findColor(){
    	logger.info("============查找年款和内饰==================");
    	List<OrganizationTreeDto> treeList = new ArrayList<>();
    	List<Map> list = forecastImportService.findModelYearList();
    	for(int i = 0; i < list.size(); i++){
    		OrganizationTreeDto dto = new OrganizationTreeDto();
    		dto.setId(CommonUtils.checkNull(list.get(i).get("MODEL_YEAR")));
    		dto.setText(CommonUtils.checkNull(list.get(i).get("MODEL_YEAR")));
    		dto.setParent("#");
    		treeList.add(dto);
    	}
    	for(int i = 0; i < list.size(); i++){
    		String modelYear = CommonUtils.checkNull(list.get(i).get("MODEL_YEAR"));
    		List<Map> colorList = forecastImportService.findGroupByModelYear(modelYear);
    		for(int j = 0; j < colorList.size(); j++){
    			OrganizationTreeDto dto = new OrganizationTreeDto();
    			dto.setId(CommonUtils.checkNull(colorList.get(j).get("GROUP_ID")));
    			dto.setText(CommonUtils.checkNull(colorList.get(j).get("GROUP_NAME")));
    			dto.setParent(modelYear);
    			treeList.add(dto);
    		}
    	}
		return treeList;
     ...
     * @return
     */
    @RequestMapping(value="/findColorAndTrim", method=RequestMethod.GET)
    @ResponseBody
    public List<Map> findColorAndTrim(@RequestParam Map<String,String> params){
    	List<Map> dto = forecastImportService.findColorAndTrim(params);
		return dto;   	
    }
     ...
     * @return
     */
    @RequestMapping(value="/saveColorAndTrim", method=RequestMethod.GET)
    @ResponseBody
    public boolean saveColorAndTrim(@RequestParam Map<String,String> params){
    	boolean flag = forecastImportService.saveColorAndTrim(params);
		return flag; 
    }
    
    @RequestMapping(value="/delColorAndTrim/{materialId}", method=RequestMethod.DELETE)
    @ResponseBody
    public boolean delColorAndTrim(@PathVariable Long materialId){
    	boolean flag = forecastImportService.delColorAndTrim(materialId);
		return flag; 
    }
