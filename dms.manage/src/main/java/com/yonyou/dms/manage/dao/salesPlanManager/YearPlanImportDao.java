	...
	* @throws
	...
	public ImportResultDto<TmpVsYearlyPlanDTO> checkData(TmpVsYearlyPlanDTO rowDto) {
		...
		for(int i=0; i < tvypList.size(); i++){
			...
			// 取得行号
			Integer rowNum = Integer.valueOf(CommonUtils.checkNull( tvypMap.get("ROW_NUMBER")));
			// 校验合计
	...
	private String checkSumAmt(Map<String,Object> map) {
		...
		try {
			Integer janAmt = Integer.valueOf(CommonUtils.checkNull(map.get("JAN_AMT")));
			sumTmp += janAmt;
		} catch (Exception e) {
			errors.append("1月数量必须是整数,");
		}
		try {
			Integer febAmt = Integer.valueOf(CommonUtils.checkNull(map.get("FEB_AMT")));
			sumTmp += febAmt;
		} catch (Exception e) {
			errors.append("2月数量必须是整数,");
		}
		try {
			Integer marAmt = Integer.valueOf(CommonUtils.checkNull( map.get("MAR_AMT")));
			sumTmp += marAmt;
		} catch (Exception e) {
			errors.append("3月数量必须是整数,");
		}
		try {
			Integer aprAmt = Integer.valueOf(CommonUtils.checkNull( map.get("APR_AMT"))); 
			sumTmp += aprAmt;
		} catch (Exception e) {
			errors.append("4月数量必须是整数,");
		}
		try {
			Integer mayAmt = Integer.valueOf(CommonUtils.checkNull( map.get("MAY_AMOUNT"))); 
			sumTmp += mayAmt;
		} catch (Exception e) {
			errors.append("5月数量必须是整数,");
		}
		try {
			Integer junAmt = Integer.valueOf(CommonUtils.checkNull( map.get("JUN_AMT"))); 
			sumTmp += junAmt;
		} catch (Exception e) {
			errors.append("6月数量必须是整数,");
		}
		try {
			Integer julAmt = Integer.valueOf(CommonUtils.checkNull( map.get("JUL_AMT"))); 
			sumTmp += julAmt;
		} catch (Exception e) {
			errors.append("7月数量必须是整数,");
		}
		try {
			Integer augAmt = Integer.valueOf(CommonUtils.checkNull( map.get("AUG_AMT"))); 
			sumTmp += augAmt;
		} catch (Exception e) {
			errors.append("8月数量必须是整数,");
		}
		try {
			Integer sepAmt = Integer.valueOf(CommonUtils.checkNull( map.get("SEP_AMT")));
			sumTmp += sepAmt;
		} catch (Exception e) {
			errors.append("9月数量必须是整数,");
		}
		try {
			Integer octAmt = Integer.valueOf(CommonUtils.checkNull( map.get("OCT_AMT")));
			sumTmp += octAmt;
		} catch (Exception e) {
			errors.append("10月数量必须是整数,");
		}
		try {
			Integer novAmt = Integer.valueOf(CommonUtils.checkNull( map.get("NOV_AMT"))); 
			sumTmp += novAmt;
		} catch (Exception e) {
			errors.append("11月数量必须是整数,");
		}
		try {
			Integer decAmt = Integer.valueOf(CommonUtils.checkNull( map.get("DEC_AMT"))); 
			sumTmp += decAmt;
		} catch (Exception e) {
		...
		if (errors.length() == 0) {
			...
			if (sumTmp.doubleValue() != Double.valueOf(CommonUtils.checkNull( map.get("SUM_AMT")))) {
				errors.append("合计错误,");
			}
		}
