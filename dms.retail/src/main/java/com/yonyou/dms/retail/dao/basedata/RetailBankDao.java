...
public class RetailBankDao extends OemBaseDAO {
	...
	public List<Map> checkRight() {
		...
		sql.append(" union  \n");
		sql.append("select ROW_NO ,'成交价不能小于0' ERR_DATA  \n");
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 <> TRIM(trdt.NET_PRICE REGEXP '[^0-9.]') \n");
		sql.append("union  \n");
		sql.append("select ROW_NO ,'首付数据不能小于0' ERR_DATA \n");
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 <> TRIM(trdt.FIRST_PERMENT REGEXP '[^0-9.]') \n");
		sql.append("union  \n");
		sql.append("select ROW_NO ,'贷款金额数据不能小于0' ERR_DATA   \n"); 
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 <> TRIM(trdt.DEAL_AMOUNT REGEXP '[^0-9.]') \n");
		sql.append("union  \n");
		sql.append("select ROW_NO ,'尾款数据不能小于0' ERR_DATA  \n");
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 <> TRIM(trdt.END_PERMENT REGEXP '[^0-9.]') \n");
		sql.append("union  \n");
		sql.append("select ROW_NO ,'总利息（手续费）数据不能小于0' ERR_DATA \n"); 
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 <> TRIM(trdt.TOTAL_INTEREST REGEXP '[^0-9.]') \n");
		sql.append("union  \n");
		sql.append("select ROW_NO ,'贴息金额 数据不能小于0' ERR_DATA   \n");
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 <> TRIM(trdt.ALLOWANCED_SUM_TAX REGEXP '[^0-9.]') \n");
		sql.append("union  \n");
		
		
		sql.append("select ROW_NO, '首付比例不在0~1之间' ERR_DATA  from( select trdt.ROW_NO ,trdt.FIRST_PERMENT_RATIO \n");
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 = TRIM(trdt.FIRST_PERMENT_RATIO REGEXP '[^0-9.]') \n");
		sql.append(" ) t WHERE CAST(FIRST_PERMENT_RATIO AS UNSIGNED INT) < 0 OR CAST(FIRST_PERMENT_RATIO AS UNSIGNED INT ) >=1   \n");
		sql.append(" union  \n");
		sql.append("select ROW_NO, '尾款比例不在0~1之间' ERR_DATA  from( select trdt.ROW_NO ,trdt.END_PERMENT_RATIO \n");
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 = TRIM(trdt.END_PERMENT_RATIO REGEXP '[^0-9.]') \n");
		sql.append(" ) t WHERE CAST(END_PERMENT_RATIO AS UNSIGNED INT) < 0 OR CAST(END_PERMENT_RATIO AS UNSIGNED INT ) >=1   \n");
		sql.append(" union  \n");
		sql.append("select ROW_NO, '原利率不在0~1之间' ERR_DATA  from( select trdt.ROW_NO ,trdt.INTEREST_RATE \n");
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 = TRIM(trdt.INTEREST_RATE REGEXP '[^0-9.]') \n");
		sql.append(")  t WHERE CAST(INTEREST_RATE AS UNSIGNED INT) < 0 OR CAST(INTEREST_RATE AS UNSIGNED INT ) >=1   \n");
		sql.append(" union  \n");
		sql.append("select ROW_NO, '政策费率不在0~1之间' ERR_DATA  from( select trdt.ROW_NO ,trdt.POLICY_RATE \n");
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 = TRIM(trdt.POLICY_RATE REGEXP '[^0-9.]') \n");
		sql.append(")  t WHERE CAST(POLICY_RATE AS UNSIGNED INT) < 0 OR CAST(POLICY_RATE AS UNSIGNED INT ) >=1   \n");
		sql.append(" union  \n");
		sql.append("select ROW_NO, '商户手续费率(客户利率)不在0~1之间' ERR_DATA  from( select trdt.ROW_NO ,trdt.MERCHANT_FEES_RATE \n");
		sql.append("from TM_RETAIL_DISCOUNT_BANK_IMPORT_TEMP trdt  where 0 = TRIM(trdt.MERCHANT_FEES_RATE REGEXP '[^0-9.]') \n");
		sql.append(" ) t WHERE CAST(MERCHANT_FEES_RATE AS UNSIGNED INT) < 0 OR CAST(MERCHANT_FEES_RATE AS UNSIGNED INT ) >=1   \n");
