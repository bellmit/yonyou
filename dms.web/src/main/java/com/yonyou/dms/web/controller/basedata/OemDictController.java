...
@Controller
@TxnConn
@RequestMapping("/basedata/oemDicts")
public class OemDictController extends BaseController{
	private static final Logger logger = LoggerFactory.getLogger(DictController.class);
	
	@Resource(name="OemDictCache")
	CacheService<List<OemDictDto>> dictCacheSerivce;
	
	@Resource(name="OemRegionCache")
    CacheService<OemRegionDto> regionCacheService;
	...
	* @return Map<String, Map<String,List<DictDto>>>
	...
	public Map<String, Object> queryDicts(){
		...
		Map<?,OemRegionDto> regionList = regionCacheService.getAllData();
		Map<?, List<OemDictDto>> dictList = dictCacheSerivce.getAllData();
		dict.put("oemDict", dictList);
		dict.put("oemRegion", regionList);
		return dict;
	}
}
