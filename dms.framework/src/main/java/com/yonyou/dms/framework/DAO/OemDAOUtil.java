...
public class OemDAOUtil {

	// 定义日志接口
	private static final Logger logger = LoggerFactory.getLogger(OemDAOUtil.class);
	 ...
	 * @return
	 */
	private static List<Map> commonFindAll(String sql, List queryParam, boolean isAclCheck,
			DefinedRowProcessor defindProcessor, String menuId) {
		...
		logger.info("Sql:" + sqlFinal + ";" + Arrays.toString(queryParamList.toArray()));
	 ...
	 * @return
	 */
	public static List<Map> downloadPageQuery(String sql, List queryParam) {
		return downloadCommonPageQuery(sql, queryParam, null, null).getRows();
	}
	...
	private static PageInfoDto commonPageQuery(String sql, List queryParam, DefinedRowProcessor processor,
		...
		Integer pageSize ;
		if(requestPageInfoDto.getLimit() == null){
			pageSize = 5;
		}else{
			pageSize = Integer.parseInt(requestPageInfoDto.getLimit());
		}
		String sort = requestPageInfoDto.getSort();
		String order = requestPageInfoDto.getOrder();
		Integer offset ;
		if(requestPageInfoDto.getOffset() == null){
			offset = 0 ;
		}else{
			offset = Integer.parseInt(requestPageInfoDto.getOffset());
		}
	 ...
	 * @return
	 */
	private static PageInfoDto downloadCommonPageQuery(String sql, List queryParam, DefinedRowProcessor processor,
			String menuId) {
		List queryParamList = queryParam;
		// 创建queryParam 对象
		if (queryParam == null) {
		...
		// 获取分页信息
		RequestPageInfoDto requestPageInfoDto = ApplicationContextHelper.getBeanByType(RequestPageInfoDto.class);
		Integer pageSize = OemDictCodeConstants.DOWNLOAD_PAGE_SIZE_MAX;
		String sort = requestPageInfoDto.getSort();
		String order = requestPageInfoDto.getOrder();
		int page = 1;
	...
	// if (!StringUtils.isNullOrEmpty(loginInfo) &&
	// !StringUtils.isNullOrEmpty(loginInfo.getDealerCode())) {
	// String dealerCode = loginInfo.getDealerCode();
	// StringBuilder sbFinal = new StringBuilder();
	// sbFinal.append("select * from (").append(sql).append(") tt where
	// DEALER_CODE in
	// (?,'").append(CommonConstants.PUBLIC_DEALER_CODE).append("')");
	...
	public static String getDealerAclTableSql(String tableName, List queryParam) {
		...
		if (!StringUtils.isNullOrEmpty(loginInfo) && !StringUtils.isNullOrEmpty(loginInfo.getDealerCode())) {
			String dealerCode = loginInfo.getDealerCode();
			StringBuilder sbFinal = new StringBuilder();
			sbFinal.append("(select * from ").append(tableName).append(" where DEALER_CODE in (?,'")
					.append(CommonConstants.PUBLIC_DEALER_CODE).append("'))");
			queryParam.add(dealerCode);
	...
	private static String getUserAclSql(String sql, List queryParam, Map<String, Object> dataRange) {
		...
		if (!StringUtils.isNullOrEmpty(loginInfo) && !StringUtils.isNullOrEmpty(loginInfo.getDealerCode())) {
			...
			sbFinal.append("select * from (").append(sql).append(") tt where DEALER_CODE in (?,'")
					.append(CommonConstants.PUBLIC_DEALER_CODE).append("')");
			queryParam.add(dealerCode);
	 ...
	 * @return
	 */
	public static Long getOemCompanyId(LoginInfoDto loginUser) {
		Long companyId=null;
		if(loginUser==null)
		{
			return null;	
