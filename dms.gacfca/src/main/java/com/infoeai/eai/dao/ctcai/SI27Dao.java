...
@Repository
public class SI27Dao extends OemBaseDAO {

	/**
	 * 功能说明:期货订单导入——TI_FUTUREORDER_IMPORT
	 * @return
	 */
	public static List<Dcs2Ctcai02DTO> getSI27Info(String string) {
		StringBuffer sql = new StringBuffer("");
		
		sql.append(" SELECT tsi.SEQUENCE_ID,tsi.VIN, \n");
		sql.append("        'QHDD' ACTION_CODE, \n");
		sql.append("        DATE_FORMAT(tsi.CREATE_DATE, '%Y%m%d') ACTION_DATE, \n");
		sql.append("        DATE_FORMAT (tsi.CREATE_DATE, '%H%i%S') ACTION_TIME, \n");
		sql.append("        (SELECT ctcai_code FROM TM_COMPANY WHERE company_code = tsi.ORDER_DEALER) ORDER_DEALER, \n");
		sql.append("        (SELECT DEALER_NAME FROM TM_DEALER WHERE dealer_code = tsi.ORDER_DEALER) DEALER_NAME, \n");
		...
		sql.append("   FROM TI_FUTUREORDER_IMPORT_DCS tsi,  \n");
	...
	private static List<Dcs2Ctcai02DTO> setDcs2Ctcai02DTOList(List<Map> mapList) {
		List<Dcs2Ctcai02DTO> list = new ArrayList<>();
