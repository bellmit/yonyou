...
@Service
public class YORDExecutorImpl extends BaseService implements YORDExecutor {
    
	private static Logger logger = LoggerFactory.getLogger(YORDExecutorImpl.class);
	...
	public String execute(ISAPOutBoundVO dto) throws Exception {
		...
		try {
			...
			if (swt2Dcs01Check(nodeId, sapOutboundDTO)) {
				...
				if (flagVehicleList!=null && flagVehicleList.size() > 0) {
					// 做更新操作
				...
				logger.info("============YORD校验失败==================");
			}
			// 期货资源预定
			int total = ISAPOutBoundCommon.methodToFuturesResource(sapOutboundDTO.getVin());
			dbService.endTxn(true);
			logger.info("-----调用期货资源设定规则结束,共有" + total + "条数据进入公共资源池-----");
		} catch (Throwable e) {
			logger.info("=============YORD处理异常============");
			logger.error(e.getMessage(), e);
			dbService.endTxn(false);
			//messg = Utility.getErrorStack(e, EAIConstant.ERROR_STACK_MAX_LENGTH);
			throw new Exception("============YORD处理异常=================="+e);
		} finally {
			Base.detach();
			dbService.clean();
			if(messg != null && !messg.equals("")){
				logger.info("=============YORD插入或更新业务表失败，记录错误信息到接口日志表开始============");
				//EaiUtil.updTiNodeDetial(nodeId, messg);
				logger.info("=============YORD插入或更新业务表失败，记录错误信息到接口日志表完成============");
			}
		}
		logger.info("============YORD处理结束==================");
		//返回处理结果
		if (flag) {
			return "02";
		} else {
	...
	public boolean isValid(ISAPOutBoundVO dto) throws Exception {
		try {
			return "YORD".equals(dto.getActionCode());
		} catch (Exception e) {
			throw new Exception("============YORD的 actionCode验证异常=================="+e);
		}
	}
	...
	private boolean insertMethod(SapOutboundVO sapOutboundDTO, Long nodeId) throws Exception{
		try {
			// 查找唯一的orgId
			List<TmOrgPO> orgList = TmOrgPO.find(" ORG_TYPE = ? and PARENT_ORG_ID = ? ", OemDictCodeConstants.ORG_TYPE_OEM, -1L);
			Long orgId = 0L;
			if(null != orgList && orgList.size() > 0){
				orgId = orgList.get(0).getLong("ORG_ID");
			}
			/* 1.操作车辆表 */
			TmVehiclePO vehicle = new TmVehiclePO();
			vehicle.setInteger("ORG_TYPE", OemDictCodeConstants.ORG_TYPE_OEM);
...
//					material.setLong("MATERIAL_ID", tempMatId);
			    ...
			    conVehc.setInteger("LIFE_CYCLE", 11521001);
