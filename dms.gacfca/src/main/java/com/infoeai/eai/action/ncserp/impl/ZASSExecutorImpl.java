 ...
 */
@Service
public class ZASSExecutorImpl extends BaseService implements ZASSExecutor{
    
	private static Logger logger = LoggerFactory.getLogger(ZASSExecutorImpl.class);
	...
	public String execute(ISAPOutBoundVO dto) throws Exception {
		...
		try {
			...
			logger.info("=============ZASS处理异常============");
			logger.error(e.getMessage(), e);
			dbService.endTxn(false);
			//messg = Utility.getErrorStack(e, EAIConstant.ERROR_STACK_MAX_LENGTH);
			throw new Exception("============ZASS处理异常=================="+e);
		}finally {
			Base.detach();
			dbService.clean();
			if(messg != null && !messg.equals("")){
				logger.info("=============ZASS插入或更新业务表失败，记录错误信息到接口日志表开始============");
				//EaiUtil.updTiNodeDetial(nodeId, messg);
				logger.info("=============ZASS插入或更新业务表失败，记录错误信息到接口日志表完成============");
			}
		}
		logger.info("============ZASS处理结束==================");
		//返回处理结果
		if (flag) {
			return "02";
		} else {
	...
	public boolean isValid(ISAPOutBoundVO vo) throws Exception{
		try {
			return "ZASS".equals(vo.getActionCode());
		} catch (Exception e) {
