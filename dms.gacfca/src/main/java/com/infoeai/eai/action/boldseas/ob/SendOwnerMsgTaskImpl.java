...
@Service
public class SendOwnerMsgTaskImpl extends BaseService implements SendOwnerMsgTask{
	
	private static final Logger logger = LoggerFactory.getLogger(SendOwnerMsgTaskImpl.class);
	
	public String execute() throws Exception {
		List<TtWarrantyRegistrationInterfaceHistoryPO> tempList = new ArrayList<TtWarrantyRegistrationInterfaceHistoryPO>();
		try {
			...
			for(int i = 0;i<list.size();i++){
				infoKey = list.get(i);
				TtWarrantyRegistrationInterfaceInfoPO infoPo = TtWarrantyRegistrationInterfaceInfoPO.
						findFirst(" DMS_OWNER_ID =? AND DEALER_CODE = ? AND VIN = ? ", infoKey.getDmsOwnerId(),infoKey.getDealerCode(),infoKey.getVin());
				...
				historyPo.setString("SEND_STATUS",infoPo.getInteger("DCS_SEND_BOLD_STATUS").toString());
...
//				factory.insert(tempList); //save保修登记信息
				for (TtWarrantyRegistrationInterfaceHistoryPO po : tempList) {
					po.saveIt();
				}
	...
	private List<SendBoldVO> getTtWarrantyRegistrationInterfaceInfoList(){
		List<SendBoldVO> resultList = new ArrayList<SendBoldVO>();
...
//		sql.append("  AND DATE(SYSDATE) - DATE( T.BUY_TIME) = 1 \n");
	...
	public static void main(String[] args) throws Exception {
		SendOwnerMsgTaskImpl task = new SendOwnerMsgTaskImpl();
		task.execute();
	}
