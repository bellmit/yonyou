 ...
 */
@Service
public class ZVP8ExecutorImpl extends BaseService implements ZVP8Executor {
    
	private static Logger logger = LoggerFactory.getLogger(ZVP8ExecutorImpl.class);
...
//			dbService();
			...
			if (swt2Dcs01Check(nodeId, sapOutboundDTO)) {
				// 判断车辆是否存在
				List<TmVehiclePO> flagVehicleList = TmVehiclePO.find(" VIN=? ", vin);
			...
			logger.info("=============ZVP8处理异常============");
			logger.error(e.getMessage(), e);
			dbService.endTxn(false);
			//messg = Utility.getErrorStack(e, EAIConstant.ERROR_STACK_MAX_LENGTH);
			throw new Exception("============ZVP8处理异常=================="+e);
		}finally {
			Base.detach();
			dbService.clean();
			if(messg != null && !messg.equals("")){
				logger.info("=============ZVP8插入或更新业务表失败，记录错误信息到接口日志表开始============");
				//EaiUtil.updTiNodeDetial(nodeId, messg);
				logger.info("=============ZVP8插入或更新业务表失败，记录错误信息到接口日志表完成============");
			}
		}
		logger.info("============ZVP8处理结束==================");
		//返回处理结果
		if (flag) {
			return "02";
		} else {
	...
	public boolean isValid(ISAPOutBoundVO vo) throws Exception{
		try {
			return "ZVP8".equals(vo.getActionCode());
		} catch (Exception e) {
			throw new Exception("============ZVP8的actionCode验证处理异常=================="+e);
		}
	}
...
//			int nodeStatus = Integer.parseInt(siComDAO.getCodeId(factory,sapOutboundVO.getActionCode(), "节点状态")); // 节点状态
			...
			if((currentDate > historyDate)){
				...
				vehicle.setInteger("LIFE_CYCLE", OemDictCodeConstants.LIF_CYCLE_02);
				vehicle.setTimestamp("ARRIVE_PORT_DATE",nodeDate); // 车辆表的到港日期
