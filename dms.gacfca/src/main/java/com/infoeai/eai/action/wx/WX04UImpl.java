...
@Service
public class WX04UImpl extends BaseService implements WX04U {
	...
	public String handleExecute() throws Exception {
		logger.info("====WX04u is begin====");
		// 查找结果集
		Map tcmp = null;
		int sendTimes = 0;
		
		List<Map> resultList = dao.getWX04uInfo();
		...
		for (int i = 0; i < resultList.size(); i++) {
			...
			}catch(SocketTimeoutException ex)
			{
				...
				logger.info("====WX04u SocketTimeout break====");
				break;
			} catch (Exception ex) {
				logger.error(ex.getMessage(), ex);
	...
	public Map<String, String> bulidParam4Send(Map po)
		...
		paramMap.put("isValid", CommonUtils.checkNull(po.get("IS_VALID")));
		paramMap.put("isAccountMgr", CommonUtils.checkNull(po.get("IS_SERVICE_ADVISOR")));
