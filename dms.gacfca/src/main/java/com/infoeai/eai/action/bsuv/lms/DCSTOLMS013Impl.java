...
/**
 * (DCS-LMS)官网订单实销信息推送接口
 * @author wangJian
 * date 2016-04-29
 *
 */
@Service
public class DCSTOLMS013Impl extends BaseService implements DCSTOLMS013{
	
	private static final Logger logger = LoggerFactory.getLogger(DCSTOLMS013Impl.class);
	@Autowired
	BSUVDCSTOLMSDAO dao;
	...
	public ResaleOrderDcsToLmsVO[] orderSalesInfo(String FROM,String TO) throws Exception {
		...
		String excString="";
		try {
			logger.info("====官网订单实销信息推送 is begin====");
			String statu = "13011035";
			...
			logger.info("====官网订单实销信息推送  is error====");
			e.printStackTrace();
			...
			beginDbService();
			try {
				CommonBSUV.insertTtEcOrderHistory(this.getClass().getName(), "(DCS-LMS)官网订单实销信息推送", startDate, list.size(), ifType, excString, "", "", new Date());
				dbService.endTxn(true);
			} catch (Exception e2) {
				dbService.endTxn(false);
				logger.info("================(DCS-LMS)官网订单实销信息推送=========");
			}finally {
				Base.detach();
				dbService.clean();
			}
			logger.info("====官网订单实销信息推送 is finish====");
		}
		return voList;
	}
