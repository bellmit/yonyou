...
/**
 * (DCS-LMS)官网批售订单发车出库状态推送接口
 * @author wangJian
 * date 2016-04-29
 *
 */
@Service
public class DCSTOLMS011Impl extends BaseService implements DCSTOLMS011{
	
	private static final Logger logger = LoggerFactory.getLogger(DCSTOLMS011Impl.class);
	...
	public CloseOutOrderDcsToLmsVO[] orderDepartChuKuState(String FROM,String TO) throws Exception {
		...
		String excString="";
		try {
			logger.info("====官网批售订单发车出库状态推送 is begin====");
			list = dao.getOrderState(FROM,TO,OemDictCodeConstants.K4_VEHICLE_NODE_17);
			...
			for(int i =0;i<list.size();i++){
				...
				vo.setETA_DATE(CommonUtils.checkNull(list.get(i).get("ETA_DATE")));
				voList[i] = vo;
				
				TtVsOrderPO tvOrder = TtVsOrderPO.findFirst("EC_ORDER_NO = ? ", CommonUtils.checkNull(list.get(i).get("EC_ORDER_NO")));
				tvOrder.setTimestamp("DESPATCH_LMS_DATE",new Date());
			...
			logger.info("====官网批售订单发车出库状态推送 is error====");
			e.printStackTrace();
			...
			beginDbService();
			try {
				CommonBSUV.insertTtEcOrderHistory(this.getClass().getName(), "(DCS-LMS)官网批售订单发车出库状态推送接口", startDate, list.size(), ifType, excString, "", "", new Date());
				dbService.endTxn(true);
			} catch (Exception e2) {
				dbService.endTxn(false);
				logger.info("================(DCS-LMS)官网批售订单发车出库状态推送接口=========");
			}finally {
				Base.detach();
				dbService.clean();
			}
			logger.info("====官网批售订单发车出库状态推送 is finish====");
		}
		return voList;
	}
