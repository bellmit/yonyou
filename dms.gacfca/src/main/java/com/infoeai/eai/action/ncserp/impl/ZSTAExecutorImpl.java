 ...
 */
@Service
public class ZSTAExecutorImpl extends BaseService implements ZSTAExecutor {
    
	private static Logger logger = LoggerFactory.getLogger(ZSTAExecutorImpl.class);
	...
	public String execute(ISAPOutBoundVO dto) throws Exception {
		...
		try {
			...
			logger.info("=============ZSTA处理异常============");
			logger.error(e.getMessage(), e);
			dbService.endTxn(false);
			//messg = Utility.getErrorStack(e, EAIConstant.ERROR_STACK_MAX_LENGTH);
			throw new Exception("============ZSTA处理异常=================="+e);
		}finally {
			Base.detach();
			dbService.clean();
			if(messg != null && !messg.equals("")){
				logger.info("=============ZSTA插入或更新业务表失败，记录错误信息到接口日志表开始============");
				//EaiUtil.updTiNodeDetial(nodeId, messg);
				logger.info("=============ZSTA插入或更新业务表失败，记录错误信息到接口日志表完成============");
			}
		}
		logger.info("============ZSTA处理结束==================");
		//返回处理结果
		if (flag) {
			return "02";
		} else {
	...
	public boolean isValid(ISAPOutBoundVO vo) throws Exception{
		try {
			return "ZSTA".equals(vo.getActionCode());
		} catch (Exception e) {
			throw new Exception("============ZASS的actionCode验证处理异常=================="+e);
		}
	}
