 ...
 */
@Service
public class ZSHPExecutorImpl extends BaseService implements ZSHPExecutor{
    
	private static Logger logger = LoggerFactory.getLogger(ZSHPExecutorImpl.class);
	...
	public String execute(ISAPOutBoundVO dto) throws Exception {
		...
		try {
			...
			logger.info("=============ZSHP处理异常============");
			logger.error(e.getMessage(), e);
			dbService.endTxn(false);
			//messg = Utility.getErrorStack(e, EAIConstant.ERROR_STACK_MAX_LENGTH);
			throw new Exception("============ZSHP处理异常=================="+e);
		}finally {
			Base.detach();
			dbService.clean();
			if(messg != null && !messg.equals("")){
				logger.info("=============ZSHP插入或更新业务表失败，记录错误信息到接口日志表开始============");
				//EaiUtil.updTiNodeDetial(nodeId, messg);
				logger.info("=============ZSHP插入或更新业务表失败，记录错误信息到接口日志表完成============");
			}
		}
		logger.info("============ZSHP处理结束==================");
		//返回处理结果
		if (flag) {
			return "02";
		} else {
	...
	public boolean isValid(ISAPOutBoundVO dto) throws Exception {
		try {
			return "ZSHP".equals(dto.getActionCode());
		} catch (Exception e) {
			throw new Exception("============ZSHP的actionCode验证处理异常=================="+e);
		}
	}
