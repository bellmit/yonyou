 ...
 */
@Service
public class ZPDPExecutorImpl extends BaseService implements ZPDPExecutor  {
    
	private static Logger logger = LoggerFactory.getLogger(ZPDPExecutorImpl.class);
	...
	public String execute(ISAPOutBoundVO dto) throws Exception {
		...
		try {
			...
			logger.info("=============ZPDP处理异常============");
			logger.error(e.getMessage(), e);
			dbService.endTxn(false);
			//messg = Utility.getErrorStack(e, EAIConstant.ERROR_STACK_MAX_LENGTH);
			throw new Exception("============ZPDP处理异常=================="+e);
		}finally {
			Base.detach();
			dbService.clean();
			if(messg != null && !messg.equals("")){
				logger.info("=============ZPDP插入或更新业务表失败，记录错误信息到接口日志表开始============");
				//EaiUtil.updTiNodeDetial(nodeId, messg);
				logger.info("=============ZPDP插入或更新业务表失败，记录错误信息到接口日志表完成============");
			}
		}
		logger.info("============ZPDP处理结束==================");
		//返回处理结果
		if (flag) {
			return "02";
		} else {
	...
	public boolean isValid(ISAPOutBoundVO vo) throws Exception{
		try {
			return "ZPDP".equals(vo.getActionCode());
		} catch (Exception e) {
...
//			int nodeStatus = Integer.parseInt(siComDAO.getCodeId(factory,sapOutboundVO.getActionCode(), "节点状态")); // 节点状态
			...
			vehicle.setDate("ORG_STORAGE_DATE", nodeDate); // ZPDP时间
			//插入港口信息到车辆表  add by dwl 20150514
...
//					material.setLong("MATERIAL_ID", tempMatId);
			    ...
			    conVehc.setDate("ORG_STORAGE_DATE", nodeDate); // ZPDP时间
				//插入港口信息到车辆表  add by dwl 20150514
			    if (null != sapOutboundDTO.getPortofdestination()) {
					if (sapOutboundDTO.getPortofdestination().equals("13922")) {
