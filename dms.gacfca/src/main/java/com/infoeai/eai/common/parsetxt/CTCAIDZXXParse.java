...
public class CTCAIDZXXParse extends TXTFactory {
	public static Logger logger = Logger.getLogger(SICommonDao.class);
	ReadFileTxt rdt = new ReadFileTxt();
	
	@Override
	public List<Ctcai2DcsDZXXDTO> readTxt(String filePath,String childFile,String fileName) {
		
		rdt.copyFile(filePath, "E:/Interface/ctcai/out/DZXX/Archive/"+fileName);
		List<Ctcai2DcsDZXXDTO> dcp = new ArrayList<Ctcai2DcsDZXXDTO>();
		// 获取txt信息
		...
		if (str != null && str.length() != -1) {
			...
			try {
				...
				for (int i = 1; i < row.length - 1; i++) {
					...
					Ctcai2DcsDZXXDTO po = new Ctcai2DcsDZXXDTO();
					...
					po.setNumber(CommonUtils.checkNull(str2[3].trim()));
					dcp.add(po);
				}
			} catch (Exception e) {
