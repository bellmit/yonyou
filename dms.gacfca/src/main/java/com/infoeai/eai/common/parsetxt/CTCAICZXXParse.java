...
public class CTCAICZXXParse extends TXTFactory {
	public static Logger logger = Logger.getLogger(SICommonDao.class);
	ReadFileTxt rdt = new ReadFileTxt();
	
	@Override
	public List<Ctcai2DcsCZXXDTO> readTxt(String filePath,String childFile,String fileName) {
		
		rdt.copyFile(filePath, "E:/Interface/ctcai/out/CZXX/Archive/"+fileName);
		List<Ctcai2DcsCZXXDTO> dcp = new ArrayList<Ctcai2DcsCZXXDTO>();
		// 获取txt信息
		...
		if (str != null && str.length() != -1) {
			...
			try {
				...
				for (int i = 1; i < row.length - 1; i++) {
					...
					Ctcai2DcsCZXXDTO po = new Ctcai2DcsCZXXDTO();
					po.setVin(CommonUtils.checkNull(str2[0].trim()));
					po.setDealerCode(CommonUtils.checkNull(str2[1].trim()));
					po.setReachDate(CommonUtils.checkNull(str2[2].trim()));
					po.setOrderDate(CommonUtils.checkNull(str2[3].trim()));
					po.setInspectionDate(CommonUtils.checkNull(str2[4].trim()));
					po.setScanningNumber(CommonUtils.checkNull(str2[5].trim()));
					po.setPdfFileName(CommonUtils.checkNull(str2[6].trim()));
					po.setInspectionNumber(CommonUtils.checkNull(str2[7].trim()));
					po.setInspectionPDFfilename(CommonUtils.checkNull(str2[8].trim()));
					dcp.add(po);
				}
			} catch (Exception e) {
