...
@Service
public class SADMS114Impl implements SADMS114 {
            ...
            if (list != null && list.size() > 0) {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (!StringUtils.isNullOrEmpty(vo.getUpdateStatus()) && ("10041002").equals(vo.getUpdateStatus().toString())) {//新增
                        if(!StringUtils.isNullOrEmpty(vo.getBankCode())) {
                            List<Map> poBankConList = Base.findAll("SELETC * FROM tm_paying_bank WHERE DEALER_CODE='"+dealerCode+"' AND BANK_CODE='"+vo.getBankCode()+"' AND D_KEY=0 ");
