 ...
 */
@Service
public class SADMS113CoudImpl implements SADMS113Coud {
	
	final Logger logger = Logger.getLogger(SADMS113CoudImpl.class);

	@Override
	public List<OpRepairOrderDTO> getSADMS113(String dealerCode, String roNo) {
		logger.info("==========SADMS113Impl执行===========");
		try {
			List<OpRepairOrderDTO> resultList = new ArrayList();
			if (Utility.testString(roNo)) {
				...
				addRoLabour(orderVO, roLabourList);
	 ...
	 * @throws Exception
	 */
	private OpRepairOrderDTO setRepairOrder(String dealerCode, RepairOrderPO orderPO) throws Exception {
...
//		orderVO.setCancelledDate(orderPO.getDate("cancelled_Date"));
		...
		orderVO.setIsStatus(0);
		orderVO.setDeleteDate(Utility.getCurrentDateTime());
...
//		orderVO.setCardNo(orderPO.getString("card_No"));
	 ...
	 * @throws Exception
	 */
	private void addRoLabour(OpRepairOrderDTO opRepairOrderDTO, List<TtRoLabourPO> roLabourList) throws Exception {
