...
@Service
public class SEDCS112CloudImpl extends BaseCloudImpl implements SEDCS112Cloud {
	private static final Logger logger = LoggerFactory.getLogger(SEDCS112CloudImpl.class);
	...
	private void saveTiTable(List<OpRepairOrderDTO> dtos) throws Exception {
		...
		for (OpRepairOrderDTO dto : dtos) {
			...
			logger.info("====未结算工单接收上报工单数量====" + tiRepairOrderList.size());
			if (tiRepairOrderList != null && tiRepairOrderList.size() > 0) {
				...
				logger.info("====未结算工单接收上报工单状态====" + status);
				logger.info("====删除已接受未结算工单数据====" + status);
				dao.delSalesPartItem(dealerCode, roNo);
				dao.delRepairTable(dealerCode, roNo);
			}
			logger.info("====未结算工单接收上报工单接收====");
