...
@Service
public class SEDCS020CloudImpl extends BaseCloudImpl implements SEDCS020Cloud {
	
	private static final Logger logger = LoggerFactory.getLogger(SEDCS020CloudImpl.class);
	...
	public String doSend(String recallId){
		String re = "1";
		try {
			logger.info("====SEDCS020Cloud   召回活动取消下发开始====");
			//下发数据查询
			...
			if (null == dealerCodes || dealerCodes.size() == 0) {
				logger.info("====SEDCS020Cloud   召回活动取消下发结束====,无数据");
			}else{
				//获取下架经销商
				...
				logger.info("====SEDCS020Cloud  召回活动取消下发结束====,下发了(" + datalist.size() + ")条数据");
			}
		}catch (Exception e) {
			re="2";
	 ...
	 * 数据发送
	 */
	public void send(List<RecallServiceClearDTO> datalist,List<String> dealerList){
		if(null!=datalist && datalist.size()>0&&null!=dealerList && dealerList.size()>0){
			for(int i=0;i<dealerList.size();i++){
				//下发操作
//				int flag = (datalist,dealerList.get(i));
//				if(flag==1){
//					logger.info("====================SEDCS020Cloud  召回活动取消下发成功========================");
//				}else{
//					logger.info("================SEDCS020Cloud  召回活动取消下发失败====================");
//				}
			}
