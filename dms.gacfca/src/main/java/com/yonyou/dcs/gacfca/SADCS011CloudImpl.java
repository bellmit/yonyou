...
@Service
public class SADCS011CloudImpl extends BaseCloudImpl implements SADCS011Cloud {
	
	private static final Logger logger = LoggerFactory.getLogger(SADCS011CloudImpl.class);
	...
	public String receiveDate(List<SA010DTO> dtoList) throws Exception {
		logger.info("====转化率上报接收开始====");
		for (SA010DTO entry : dtoList) {
			try {
				insertData(entry);
			} catch (Exception e) {
				logger.error("转化率上报接收失败", e);
				throw new ServiceBizException(e);
			}
		}
		logger.info("====转化率上报接收结束====");
		return null;
	}
