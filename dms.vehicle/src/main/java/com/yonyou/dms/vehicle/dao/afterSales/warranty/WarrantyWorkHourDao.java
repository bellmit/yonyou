 ...
 */
@Repository
public class WarrantyWorkHourDao extends OemBaseDAO {
	 ...
	 * @return pageInfoDto
	 */
	public PageInfoDto getWorkHourQueryList(Map<String, String> queryParam) {
		List<Object> params = new ArrayList<Object>();
		String sql = getWorkHourQuerySql(queryParam, params,"query");
	 ...
	 * @return pageInfoDto
	 */
	public List<Map> getWorkHourDownloadList(Map<String, String> queryParam) {
		List<Object> params = new ArrayList<Object>();
		String sql = getWorkHourQuerySql(queryParam, params,"down");
	 ...
	 * @return
	 */
	private String getWorkHourQuerySql(Map<String, String> queryParam, List<Object> params,String operate) {
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT TWW.ID,TWW.OPT_CODE,TWW.MVS,TWW.WORK_HOUR,TC.CODE_CN_DESC AS STATUS,TU1.NAME AS CREATE_BY,TU2.NAME AS UPDATE_BY, \n  ");
		sql.append("  DATE_FORMAT(TWW.CREATE_DATE,'%Y-%m-%d') CREATE_DATE,DATE_FORMAT(TWW.UPDATE_DATE,'%Y-%m-%d') UPDATE_DATE   \n");
		sql.append(" FROM TT_WR_WORRKHOUR_DCS TWW  \n");
		sql.append(" LEFT JOIN TC_CODE TC ON TC.CODE_ID =TWW.STATUS  \n");
		sql.append(" LEFT OUTER JOIN Tc_user TU1 ON TU1.USER_ID=TWW.CREATE_BY	 \n");
		sql.append(" LEFT OUTER JOIN Tc_user TU2 ON TU2.USER_ID=TWW.UPDATE_BY  WHERE 1=1	\n  ");
		//MVS
        if(!StringUtils.isNullOrEmpty(queryParam.get("mvs"))){ 
        	sql.append(" 	AND TWW.MVS LIKE '%" + queryParam.get("mvs") + "%' \n");
        } 
		//操作代码
        if(!StringUtils.isNullOrEmpty(queryParam.get("optCode"))){ 
        	sql.append(" 	AND TWW.OPT_CODE LIKE '%" + queryParam.get("optCode") + "%' \n");
        }  
        if(operate.equals("down")){//下载排序
        	sql.append(" 	ORDER BY TWW.ID DESC,TWW.UPDATE_DATE DESC \n");
        }
	...
	/**
	 * 
	* @Title: insertTmpWorkHour 
	* @Description: 保修工时(导入临时表)
	* @param @param rowDto    设定文件 
	* @return void    返回类型 
	* @throws
	 */
//	public void insertTmpWorkHour(TmpWrOperateDTO twDTO) {
//		LoginInfoDto loginInfo = ApplicationContextHelper.getBeanByType(LoginInfoDto.class);
//		TmpWrOperatePO twPO = new TmpWrOperatePO();
//		//设置对象属性
//		twPO.setString("LINE_NO", twDTO.getRowNO());
//		twPO.setString("OPT_CODE", twDTO.getWorkHour());
	...
	* @throws
	...
	public ImportResultDto<TmpWrOperateDTO> checkData(TmpWrOperateDTO rowDto) {
		...
		for(int i=0; i < twList.size(); i++){
			...
			String WorkHour=(String) twMap.get("OPT_CODE");
			String optNameCn=(String) twMap.get("OPT_NAME_CN");
			
			if (WorkHour.trim().length() == 0) {
				isError = true;
				rowDto.setRowNO(new Integer(rowNum));
				rowDto.setErrorMsg("保修工时不能为空");
				twDTOList.add(rowDto);
				importResult.setErrorList(twDTOList);
			}else if (WorkHour.trim().length() != 7) {
				isError = true;
				rowDto.setRowNO(new Integer(rowNum));
				rowDto.setErrorMsg("保修工时长度有误(应为7个字符)");
				twDTOList.add(rowDto);
				importResult.setErrorList(twDTOList);
			}else if (!WorkHour.trim().matches("[0-9a-zA-Z]{7}")) {
				isError = true;
				rowDto.setRowNO(new Integer(rowNum));
				rowDto.setErrorMsg("保修工时是7位可以是字母，数字或者字母和数字的组合");
				twDTOList.add(rowDto);
				importResult.setErrorList(twDTOList);
			}
