...
@Repository
public class CheckDealerWarrantyClaimParam extends OemBaseDAO implements CheckRuleInfo{

	private static final Logger logger = LoggerFactory.getLogger(CheckDealerWarrantyClaimParam.class);
	
	@SuppressWarnings("unchecked")
	public Map<String, Object> executeCheckStep(Map<String, Object> params,
			Map<String, Object> message) {
		logger.debug("-------------------检查规则：检查特约店索赔参数（保修）开始-----------------------");
		...
		if("ERROR".equals(RET_FLAG)||!"".equals(ERROR_CODE)){
			...
			logger.debug("-------------------检查规则：检查特约店索赔参数（保修）结束-----------------------");
			return retMap;
		}else{
			...
			sqlParams.add(params.get("orderDealerId")==null?logonUser.getDealerId():params.get("orderDealerId"));
			sqlParams.add(params.get("orderOemCompanyId")==null?logonUser.getOemCompanyId():params.get("orderOemCompanyId"));
			...
			if(calimDays>approveDaysLimit){
				retMap.put("RET_FLAG", "ERROR");
				retMap.put("MESSAGE", "索赔失败，该工单已超过系统HMCI规定的索赔时间！");
				retMap.put("ERROR_CODE", "ERROR_RULE_57");
				logger.debug("-------------------检查规则：检查特约店索赔参数（保修）结束-----------------------");
				return retMap;
			}
		}
