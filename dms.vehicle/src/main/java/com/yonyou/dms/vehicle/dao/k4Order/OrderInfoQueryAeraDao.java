...
@Repository
public class OrderInfoQueryAeraDao extends OemBaseDAO {
	...
	private String getTotalQuerySql(Map<String, String> queryParam, List<Object> params) {
		...
		if (!StringUtils.isNullOrEmpty(queryParam.get("orderNo"))) {
			conditionWhere.append(" and TVO.ORDER_TYPE like '%" + queryParam.get("orderNo") + "%' \n");
		}
		...
		if (conditionWhere == null || conditionWhere.equals("")) {
			conditionWhere.append(" AND TOR.ORG_TYPE=" + OemDictCodeConstants.ORG_TYPE_OEM);
	...
	private String getDetailQuerySql(Map<String, String> queryParam, List<Object> params) {
		...
		// 订单提报日期 起始
		if (!StringUtils.isNullOrEmpty(queryParam.get("orderDateStart"))) {
			sql.append(" and tt.DEAL_ORDER_AFFIRM_DATE>=" + "date_format('" + queryParam.get("orderDateStart")
					+ "' '00:00:00'," + "'%Y-%m-%d %H:%i:%s'" + ")" + " \n");
		}
		// 订单提报日期 结束
		if (!StringUtils.isNullOrEmpty(queryParam.get("orderDateEnd"))) {
			sql.append(" and tt.DEAL_ORDER_AFFIRM_DATE <= " + "date_format('" + queryParam.get("orderDateEnd")
					+ "' '23:59:59'," + "'%Y-%m-%d %H:%i:%s'" + ")" + " \n");

		}
		// 指派分配日期 起始
		if (!StringUtils.isNullOrEmpty(queryParam.get("allotDateStart"))) {
			sql.append(" and tt.CREATE_DATE2 >=? \n");
			params.add(
					"date_format('" + queryParam.get("allotDateStart") + "' '00:00:00'," + "'%Y-%m-%d %H:%i:%s'" + ")");
		...
		if (!StringUtils.isNullOrEmpty(queryParam.get("allotDateEnd"))) {
			sql.append(" and tt.CREATE_DATE2 <=? \n");
			params.add(
					"date_format('" + queryParam.get("allotDateEnd") + "' '23:59:59'," + "'%Y-%m-%d %H:%i:%s'" + ")");
