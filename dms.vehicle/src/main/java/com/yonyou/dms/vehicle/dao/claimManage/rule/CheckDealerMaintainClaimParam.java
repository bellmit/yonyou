...
@Repository
public class CheckDealerMaintainClaimParam extends OemBaseDAO implements CheckRuleInfo{

	// 定义日志接口
	private static final Logger logger = LoggerFactory.getLogger(CheckDealerMaintainClaimParam.class);

	@SuppressWarnings("unchecked")
	public Map<String, Object> executeCheckStep(Map<String, Object> params,
			Map<String, Object> message) {
		logger.debug("-------------------检查规则：检查特约店索赔参数（保养）开始-----------------------");
		...
		if("ERROR".equals(RET_FLAG)||!"".equals(ERROR_CODE)){
			...
			logger.debug("-------------------检查规则：检查特约店索赔参数（保养）结束-----------------------");
			return retMap;
		}else{
			...
			if("".equals(dealerId)){
				...
				retMap.put("ERROR_CODE", "ERROR_RULE_31");
				logger.debug("-------------------检查规则：检查特约店索赔参数（保养）结束-----------------------");
				return retMap;
			}
			//检索特约店系统参数设置
			...
			if(dealerParamMap==null){
				...
				retMap.put("ERROR_CODE", "ERROR_RULE_32");
				logger.debug("-------------------检查规则：检查特约店索赔参数（保养）结束-----------------------");
				return retMap;
			}
            //如果参数不为空，检查页面费用是否超出
			...
			//取出页面
			double partFee=!"".equals(CommonUtils.checkNull(dto.getPartFee()))?
					Double.parseDouble(CommonUtils.checkNull(dto.getPartFee(), "0"))
			...
			double frtAmount=!"".equals(CommonUtils.checkNull(dto.getLabourFee()))?
					...
					:Double.parseDouble(params.get("labourHourValue").toString());//FRT总计
			...
			double calimDays=0;
			try {
				String balanceDate=!"".equals(CommonUtils.checkNull(dto.getBalanceDate()))?
						CommonUtils.checkNull(dto.getBalanceDate()):
							CommonUtils.checkNull(params.get("balanceDate"));
				calimDays=DateTimeUtil.dateBetween(balanceDate, DateTimeUtil.parseDateToDate(new Date()));
			} catch (ParseException e) {
				e.printStackTrace();
			}
			if(partFee>partFeeLimit){
				retMap.put("RET_FLAG", "ERROR");
				retMap.put("MESSAGE", "索赔失败，索赔单的零部件总金额超过规定的零部件总金额！");
				retMap.put("ERROR_CODE", "ERROR_RULE_33");
				logger.debug("-------------------检查规则：检查特约店索赔参数（保养）结束-----------------------");
				return retMap;
			}
			if(frtAmount>frtLimit){
				retMap.put("RET_FLAG", "ERROR");
				retMap.put("MESSAGE", "索赔失败，索赔单的工时合计超过规定的工时合计数！");
				retMap.put("ERROR_CODE", "ERROR_RULE_34");
				logger.debug("-------------------检查规则：检查特约店索赔参数（保养）结束-----------------------");
				return retMap;
			}
			if(totalFee>claimLimit){
				retMap.put("RET_FLAG", "ERROR");
				retMap.put("MESSAGE", "索赔失败，索赔单的合计金额超过规定的合计金额！");
				retMap.put("ERROR_CODE", "ERROR_RULE_35");
				logger.debug("-------------------检查规则：检查特约店索赔参数（保养）结束-----------------------");
				return retMap;
			}
			if(calimDays>approveDaysLimit){
				retMap.put("RET_FLAG", "ERROR");
				retMap.put("MESSAGE", "索赔失败，该工单已超过系统规定的索赔时间！");
				retMap.put("ERROR_CODE", "ERROR_RULE_36");
				logger.debug("-------------------检查规则：检查特约店索赔参数（保养）结束-----------------------");
				return retMap;
			}
		}
