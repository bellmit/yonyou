...
public class ResourceImportServiceImpl implements ResourceImportService {
	...
	public void insertTmp(ResourceOrderUploadDTO rowDto, String orderType) {
		...
		po.setLong("USER_ID", loginInfo.getUserId());
	...
	public void importTableAppProce() {
		...
		String orderType = dao.findOrderType1();
		// 指派 订单导入
		if (orderType.equals("1")) {
			...
			try {
				...
				TtVsUploadCommonResourcePO po = new TtVsUploadCommonResourcePO();
				List<TtVsUploadCommonResourcePO> tmpLis = TtVsUploadCommonResourcePO.find("Resource_Type",
						OemDictCodeConstants.ORDER_TYPE_03);
				for (TtVsUploadCommonResourcePO tvucP : tmpLis) {
					TmVehiclePO tempVeh = TmVehiclePO.findFirst("VIN=?", tvucP.get("VIN"));
					...
					if (!tempVeh.get("Vehicle_Usage").equals("")
							&& !tempVeh.get("Vehicle_Usage").equals(OemDictCodeConstants.VEHICLE_USE_01.toString())) {
						// TtResourceRemarkPO trPO = new
						// TtResourceRemarkPO();
						// trPO.setString("VIN",tempVeh.get("Vin"));
						List<TtResourceRemarkPO> list = TtResourceRemarkPO.find("VIN=?", tempVeh.get("Vin"));
						if (list.size() > 0) {
					...
					if (tempVeh.get("Node_Status").equals(OemDictCodeConstants.VEHICLE_NODE_18)) {
						// 查询是否已下发公共资源
						List<Map> tvcrList = dao.findZdrrList2(tvucP.get("VIN").toString());
						...
						if (tvcrList.size() > 0) {
							...
							TtVsCommonResourcePO.update("Status=?,Update_Date=?", "Vehicle_Id=? and Status=?",
									OemDictCodeConstants.COMMON_RESOURCE_STATUS_03, format, tempVeh.get("Vehicle_Id"),
					...
					List<TmOrgPO> tmList = TmOrgPO.find("Org_Type=? and Org_Code=?", OemDictCodeConstants.ORG_TYPE_OEM,
					...
					if (tmList.size() > 0) {
						...
						if (tvList.size() > 0) {
							...
							TtVsOrderPO.update("Order_Status=?,Update_By=?,Update_Date=?", "VIN=?",
									OemDictCodeConstants.ORDER_STATUS_08, loginInfo.getUserId(), format,
									tvucP.get("VIN"));
							// TtVsAppointOrderDcsPO tvaPO = new
							// TtVsAppointOrderDcsPO();
							TtVsAppointOrderDcsPO.update("Status=?,Update_Date=?", "ORDER_ID=?and",
									OemDictCodeConstants.STATUS_ENABLE, format,
									new Long(map1.get("ORDER_ID").toString()));
							// 更新车辆表 车辆状态为ZBIL

							TmVehiclePO.update(
									"Org_Type=?,Org_Id=?,Node_Status=?,Node_Date=?,Update_By=?,Update_Date=?", "VIN=?",
									OemDictCodeConstants.ORG_TYPE_OEM, new Long(0),
									OemDictCodeConstants.VEHICLE_NODE_08, format, loginInfo.getUserId(), format,
									tempVeh.get("Vin"));
							// 记录日志
							TtVsMatchCheckPO mcPo = new TtVsMatchCheckPO(); // 匹配更改日志表
							// mcPo.setId(Long.parseLong(SequenceManager.getSequence(null)));
							mcPo.setLong("Order_Id", new Long(map1.get("ORDER_ID").toString()));
							mcPo.setLong("Old_Vehicle_Id", (new Long(map1.get("VEHICLE_ID").toString())));// 原车辆ID
							mcPo.setLong("Chg_Vehicle_Id", tempVeh.get("Vehicle_Id"));//
							// 改变车辆ID
							mcPo.setLong("Update_By", loginInfo.getUserId());// 操作人
							mcPo.setTimestamp("Update_Date", format);// 操作时间
						...
						// 系统生成订单号
						tvOrderPO.setInteger("Order_Type", OemDictCodeConstants.ORDER_TYPE_03);
						...
						TmVehiclePO.update(
								...
								"VIN=?", OemDictCodeConstants.ORG_TYPE_OEM, toPO.get("Org_Id"), new Long(0),
								OemDictCodeConstants.VEHICLE_NODE_18, format, loginInfo.getUserId(), format,
								tempVeh.get("Vin"));
						// 插入详细车籍
						...
						vhclChng.setString("Change_Code", OemDictCodeConstants.VEHICLE_CHANGE_TYPE_24);
						...
						List<TmDealerPO> tdList = TmDealerPO.find("Dealer_Code", tvucP.get("Resource_Scope"));
						if (tdList.size() == 0)
							continue;
						tdPO = tdList.get(0);

						List<Map> tvoList = dao.findZdrrList(tvucP.get("VIN").toString());
						if (tvoList.size() > 0) {
							...
							TtVsAppointOrderDcsPO.update("Status=?,Update_Date=?", "ORDER_ID=?",
									OemDictCodeConstants.STATUS_ENABLE, format,
									new Long(map1.get("ORDER_ID").toString()));
							// 更新车辆表 车辆状态为ZBIL
						...
						TmVehiclePO.update(
								"Org_Type=?,Org_Id=?,Dealer_Id=?,Node_Status=?,Node_Date=?,Update_By?,Update_Date=?",
								"VIN=?", OemDictCodeConstants.ORG_TYPE_DEALER, 0l, tdPO.get("Dealer_Id"),
								OemDictCodeConstants.VEHICLE_NODE_18, format, loginInfo.getUserId(), format,
								tempVeh.get("Vin"));
						// 插入详细车籍
						TtVsVhclChngPO vhclChng = new TtVsVhclChngPO();
			...
			for (int i = 0; i < tmpList.size(); i++) {
				...
				if (mList.size() > 0) {
					...
					TtVsOrderPO.update("Dealer_Id=?, Order_Type=?,Order_Status=?", "Order_Id=? and VIN=? ", new Long(0),
							new Integer(0), OemDictCodeConstants.ORDER_STATUS_08,
							new Long(map.get("ORDER_ID").toString()), tvucPO.get("Vin"));
					...
					TmVehiclePO.update(
							"Org_Type=?,Dealer_Id=?,Org_Id=?,Node_Status=?,Node_Date=?,Update_By=?,Update_Date=?",
							"VIN=?", OemDictCodeConstants.ORG_TYPE_OEM, new Long(0), new Long(0),
							OemDictCodeConstants.VEHICLE_NODE_08, format, loginInfo.getUserId(), format,
							tvucPO.get("Vin"));
					// 记录日志
					TtVsMatchCheckPO mcPo = new TtVsMatchCheckPO(); // 匹配更改日志表
				...
				tvcrd.setLong("Create_By", loginInfo.getUserId());
				...
				if (tvucPO.get("Resource_Type").toString()
					...
					TmVehiclePO.update("Node_Status=?,Node_Date=?", "vin=?", OemDictCodeConstants.VEHICLE_NODE_18,
							format, tvucPO.get("Vin"));
				}
